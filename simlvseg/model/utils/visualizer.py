...
class ImageVisualizer:
    ...
    def show_image(self, image, title=None, cmap='jet', alpha=1, colorbar=False, show_axis=False, save_path=''):
        """
    ...
    def show_images(self, images, titles=None, cmap='jet', alpha=1, colorbar=False, show_axis=False, max_cols=4, save_path=None):
        """
        可视化多张图像，支持 PIL 图像、Numpy 数组和 PyTorch 张量。
        :param images: 输入图像列表，可以是 PIL 图像、Numpy 数组或 PyTorch 张量的列表
        :param titles: 图像标题列表
        :param cmap: 颜色映射，用于灰度图像或其他模式图像（默认为jet）
        :param alpha: 透明度，默认为1
        :param colorbar: 是否显示颜色条，适用于灰度图像（默认为False）
        :param show_axis: 是否显示坐标轴（默认为False）
