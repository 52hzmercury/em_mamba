...
class InceptionDWConv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=2, cube_kernel_size=3, band_kernel_size=11, branch_ratio=1/8):
        super().__init__()
        ...
        self.dwconv_wd = nn.Conv3d(in_gc, out_gc, stride=stride, kernel_size=(1, 1, band_kernel_size), padding=(0, 0, band_kernel_size // 2),
                                   groups=in_gc)
        self.dwconv_hd = nn.Conv3d(in_gc, out_gc, stride=stride, kernel_size=(1, band_kernel_size, 1), padding=(0, band_kernel_size // 2, 0),
                                   groups=in_gc)
        self.dwconv_hw = nn.Conv3d(in_gc, out_gc, stride=stride, kernel_size=(band_kernel_size, 1, 1), padding=(band_kernel_size // 2, 0, 0),
                                   groups=in_gc)
...
class InceptionDWConv3dBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, strides=1, kernel_size=3, subunits=2, act='prelu', norm='instance', dropout=0.0):
        """
        ...
        for i in range(subunits):
            ...
            layers.append(InceptionDWConv3d(conv_in, out_channels, stride=conv_stride))
...
#     module = InceptionDWConv3dBlock(in_channels=64, out_channels=128, strides=2, kernel_size=3, subunits=1, act='prelu', norm='instance', dropout=0.0)
#
...
if __name__ == '__main__':
    block = InceptionDWConv3d(64, 32)  # 输入 C
